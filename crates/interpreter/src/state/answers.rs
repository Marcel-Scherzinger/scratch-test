...
pub(crate) struct PredefinedAnswers {
    values: Rc<[model::VariableValue]>,
    next_pos: usize,
    last_answer: model::VariableValue,
}

impl PredefinedAnswers {
    pub(crate) fn new(values: impl Into<Rc<[model::VariableValue]>>) -> Self {
        Self {
            values: values.into(),
            next_pos: 0,
    ...
    pub fn last_answer(&self) -> &model::VariableValue {
        &self.last_answer
    }
    pub fn values(&self) -> &Rc<[model::VariableValue]> {
        &self.values
    }
...
impl PredefinedAnswersReport {
    ...
    pub fn used_answers(&self) -> &[model::VariableValue] {
        self.values.get(..self.next_pos).unwrap()
    }
    pub fn unused_answers(&self) -> &[model::VariableValue] {
        self.values.get(self.next_pos..).unwrap()
    }
...
mod tests {
    ...
    fn test_empty() {
        ...
        assert_eq!(model::VariableValue::Text("".into()), q.last_answer);

        let r = PredefinedAnswersReport::from(q);
        let empty: &[model::VariableValue] = &[];
        assert_eq!(empty, r.used_answers());
        assert_eq!(empty, r.unused_answers());
        assert_eq!(0, r.next_pos);
        assert_eq!(model::VariableValue::Text("".into()), r.last_answer);
    ...
    fn test_all_used() {
        ...
        assert_eq!(model::VariableValue::Int(1), q.last_answer().clone());
        assert!(q.ask_next().is_ok());
        assert_eq!(model::VariableValue::Int(2), q.last_answer().clone());
        assert!(q.ask_next().is_ok());
        assert_eq!(model::VariableValue::Int(3), q.last_answer().clone());
        assert_eq!(3, q.next_pos);
        assert_eq!(model::VariableValue::Int(3), q.last_answer);
        let q = PredefinedAnswersReport::from(q);
        let empty: &[model::VariableValue] = &[];
        let all: &[model::VariableValue] = &[1.into(), 2.into(), 3.into()];
    ...
    fn test_some_used() {
        ...
        assert_eq!(model::VariableValue::Int(3), q.last_answer);
        let q = PredefinedAnswersReport::from(q);
        let unused: &[model::VariableValue] = &[4.into(), 5.into(), 6.into()];
        let used: &[model::VariableValue] = &[1.into(), 2.into(), 3.into()];
